...
class Blockchain:
    ...
    # Cada nodo entrará a partir de un puerto de nuestra app web de Flask
    def add_node(self, address):
        parsed_url = urlparse(address)
        self.nodes.add(parsed_url.netloc)  # netloc es el url
...
# Creando una direccion para el nodo
node_address = str(uuid4()).replace('-', '')  # Direccion unica aleatoria
...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_address, receiver='Adrian', amount=1)
...
def new_transaction():
    ...
    if not all(key in json for key in transaction_keys):
        return 'Algunos elementos de la transaccion estan faltando', 400
    index = blockchain.add_transaction(json['sender'], json['receiver'], json['amount'])
    response = {'message': f'Esta transaccion será añadida al bloque {index}'}
    return jsonify(response), 201


# Paso 3 - Descentralizar nuestra blockchain
...
def connect_node():
    ...
    response = {
        'message': 'Todos los nodos estan conectados. La LuisCoin Blockchain contiene los siguientes nodos:',
...
def replace_chain():
    ...
    if is_chain_replaced:
        ...
        response = {'message': 'Esta cadena es la más larga',
